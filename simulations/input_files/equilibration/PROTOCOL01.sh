...
inpdir="/share/PI/rondror/MD_simulations/amber/robin_input_files/equilibration"
...
echo "NVT heating..."
mpirun -np 2 --bind-to socket pmemd.cuda.MPI -O -i "$inpdir/Heat_1.mdin" \
...
echo "NTP heating..."
mpirun -np 2 --bind-to socket pmemd.cuda.MPI -O -i "$inpdir/Heat_2.mdin" \
       -o "${rev}Heat_2.mdout" -p "$prmtop" -c "${rev}Heat_1.rst" \
       -r "${rev}Heat_2.rst" -ref "${rev}Heat_1.rst" -x "${rev}Heat_2.nc"

# Equilibrate with restraint 5 on protein only for 500ps
# in the NPT ensemble, 2.5fs timestep
echo "Restraint 5 equilibration..."
mpirun -np 2 --bind-to socket pmemd.cuda.MPI -O -i "$inpdir/Eq_1.mdin" \
       -o "${rev}Eq_1.mdout" -p "$prmtop" -c "${rev}Heat_2.rst" \
       -r "${rev}Eq_1.rst" -ref "${rev}Heat_2.rst" -x "${rev}Eq_1.nc"

# Decrease restraint on protein to 4 for 500ps
# in the NPT ensemble, 2.5fs timestep
echo "Restraint 4 equilibration..."
mpirun -np 2 --bind-to socket pmemd.cuda.MPI -O -i "$inpdir/Eq_2.mdin" \
       -o "${rev}Eq_2.mdout" -p "$prmtop" -c "${rev}Eq_1.rst" \
       -r "${rev}Eq_2.rst" -ref "${rev}Eq_1.rst" -x "${rev}Eq_2.nc"

# Decrease restraint on protein to 3 for 500ps
# in the NPT ensemble, 2.5fs timestep
echo "Restraint 3 equilibration..."
mpirun -np 2 --bind-to socket pmemd.cuda.MPI -O -i "$inpdir/Eq_3.mdin" \
       -o "${rev}Eq_3.mdout" -p "$prmtop" -c "${rev}Eq_2.rst" \
       -r "${rev}Eq_3.rst" -ref "${rev}Eq_2.rst" -x "${rev}Eq_3.nc"

# Decrease restraint on protein to 2 for 500ps
# in the NPT ensemble, 2.0fs timestep
echo "Restraint 2 equilibration..."
mpirun -np 2 --bind-to socket pmemd.cuda.MPI -O -i "$inpdir/Eq_4.mdin" \
       -o "${rev}Eq_4.mdout" -p "$prmtop" -c "${rev}Eq_3.rst" \
       -r "${rev}Eq_4.rst" -ref "${rev}Eq_3.rst" -x "${rev}Eq_4.nc"

# Decrease restraint on protein to 1 for 500ps
# in the NPT ensemble, 2.0fs timestep
echo "Restraint 1 equilibration..."
mpirun -np 2 --bind-to socket pmemd.cuda.MPI -O -i "$inpdir/Eq_5.mdin" \
       -o "${rev}Eq_5.mdout" -p "$prmtop" -c "${rev}Eq_4.rst" \
       -r "${rev}Eq_5.rst" -ref "${rev}Eq_4.rst" -x "${rev}Eq_5.nc"

# Remove restraints entirely, equilibrate for 500ps
# in the NPT ensemble, 2.0fs timestep
echo "No restraint equilibration..."
mpirun -np 2 --bind-to socket pmemd.cuda.MPI -O -i "$inpdir/Eq_6.mdin" \
...
# Total of 3ns equilibration now performed.
